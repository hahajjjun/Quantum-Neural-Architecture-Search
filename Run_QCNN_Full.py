...
def Circuit(param, cfg, fixed = None):
    qml.AngleEmbedding(fixed, wires=[0,1,2,3])
    param_idx = 0
    wire1 = cfg['wire1']
    wire2 = cfg['wire2']
    ...
    for idx, (element1, element2) in enumerate(zip(wire1, wire2)):
        if element1 == 'C' or element2 == 'C':
            ...
            if query == 'CC':
                ...
                param_idx += 1
                qml.CRX(param[param_idx], [2,3])
                param_idx += 1
            elif query == 'XC':
                qml.CRX(param[param_idx], [1,0])
                param_idx += 1
                qml.CRX(param[param_idx], [3,2])
                param_idx += 1
            elif query == 'CY':
                qml.CRY(param[param_idx], [0,1])
                param_idx += 1
                qml.CRY(param[param_idx], [2,3])
                param_idx += 1
            elif query == 'YC':
                qml.CRY(param[param_idx], [1,0])
                param_idx += 1
                qml.CRY(param[param_idx], [3,2])
                param_idx += 1
            elif query == 'CZ':
                qml.CRZ(param[param_idx], [0,1])
                param_idx += 1
                qml.CRZ(param[param_idx], [2,3])
                param_idx += 1
            elif query == 'ZC':
                qml.CRZ(param[param_idx], [1,0])
                param_idx += 1
                qml.CRZ(param[param_idx], [3,2])
                param_idx += 1
        else:
            if element1 == 'X':
                qml.RX(param[param_idx], 0)
                param_idx += 1
                qml.RX(param[param_idx], 2)
                param_idx += 1
            if element1 == 'Y':
                qml.RY(param[param_idx], 0)
                param_idx += 1
                qml.RY(param[param_idx], 2)
                param_idx += 1
            if element1 == 'Z':
                qml.RX(param[param_idx], 0)
                param_idx += 1
            ...
            if element2 == 'X':
                ...
                param_idx += 1
                qml.RX(param[param_idx], 3)
                param_idx += 1
            if element2 == 'Y':
                qml.RY(param[param_idx], 1)
                param_idx += 1
                qml.RY(param[param_idx], 3)
                param_idx += 1
            if element2 == 'Z':
                qml.RZ(param[param_idx], 1)
                param_idx += 1
                qml.RZ(param[param_idx], 3)
                param_idx += 1
            if element2 == 'H':
                qml.Hadamard(1)
                qml.Hadamard(3)
    ...
    for idx, (element1, element2) in enumerate(zip(wire1, wire2)):
        if element1 == 'C' or element2 == 'C':
            ...
            if query == 'CC':
                ...
                qml.CRX(param[param_idx], [1,3])
                param_idx += 1
            elif query == 'XC':
                qml.CRX(param[param_idx], [3,1])
                param_idx += 1
            elif query == 'CY':
                qml.CRY(param[param_idx], [1,3])
                param_idx += 1
            elif query == 'YC':
                qml.CRY(param[param_idx], [3,1])
                param_idx += 1
            elif query == 'CZ':
                qml.CRZ(param[param_idx], [1,3])
                param_idx += 1
            elif query == 'ZC':
                qml.CRZ(param[param_idx], [3,1])
                param_idx += 1
        else:
            if element1 == 'X':
                qml.RX(param[param_idx], 1)
...
def cost(params, X, Y, cfg, cost_fn):
    predictions = [softmax(Circuit(params, cfg, x))[0] for x in X]
    if cost_fn == 'mse':
...
def circuit_training(X_train, Y_train, X_valid, Y_valid, X_test, Y_test, cost_fn='mse'):
    ...
    os.mkdir(f'/home/hahajjjun/Junha Park/QNAS/{architecture}')
        
    with open(f"/home/hahajjjun/Junha Park/QNAS/{architecture}/log_{architecture}.txt", "w") as file:
...
architecture = circuit_training(train_X_, train_y, valid_X_, valid_y, test_X_, test_y)
fig = device._circuit.draw(output="mpl")
fig.savefig(f'/home/hahajjjun/Junha Park/QNAS/{architecture}/{architecture}.png')